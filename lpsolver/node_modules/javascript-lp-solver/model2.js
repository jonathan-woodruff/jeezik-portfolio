/*TODO 
** To make it equitable in terms of the hours a given doctor is working, you should keep the objective function as-is and solve the model. The model decides the best paths and then chooses the doctors at random, but you can run the solution through a function that makes better decisions for ehich doctors take the prescribed paths. For example, if doctor A takes path A and doctor B takes path B, you keep the paths as is but maybe doctor A takes path B and doctor B takes path A. You'll have to compare historical solutions to total the number of hours doctors have worked in the past compared to the number of hours they desired to work up to that point, and see which doctors should take a heavier workload and which ones should take a lighter one for a given run
*/
...
const doctors = ["doc 1", "doc 2"]; //all the doctors that will be available at least one day
...
/*
//c8: assigns the number of hours of the doctor who works the most hours total
for (let h = 0; h < doctors.length; h++) {
    attribute = "c8 " + h.toString();
    constraints[attribute] =  { "max": 0 };
    key = "w";
    variables[key][attribute] = -1;
    for (let g = 0; g < days.length; g++) {
        for (let i = 1; i < n[g] + 1; i++) { //real nodes
            for (let j = 1; j < n[g] + 2; j++) { //real nodes and dummy destination node
                if (j !== i) {
                    key = "x[" + g.toString() + "][" + h.toString() + "][" + i.toString() + "][" + j.toString() + "]";
                    variables[key][attribute] = m[g][i] + c[g][i][j];
                }
            }
        } 
    }
...
/*
//c9: assigns the number of hours of the doctor who works the least hours on a given day
for (let g = 0; g < days.length; g++) {
    for (let h = 0; h < H[g].length; h++) {
        attribute = "c9 " + g.toString() + " " + h.toString();
        constraints[attribute] = { "max": 0 };
        for (let i = 1; i < n[g] + 1; i++) {
            for (let j = 1; j < n[g] + 1; j++) { //real nodes and dummy destination node
                if (j !== i) {
                    key = "x[" + g.toString() + "][" + h.toString() + "][" + i.toString() + "][" + j.toString() + "]";
                    variables[key][attribute] = -1 * (m[g][i] + c[g][i][j]);
                    key = "v[" + g.toString() + "]";
                    variables[key][attribute] = 1;
                }
            }
